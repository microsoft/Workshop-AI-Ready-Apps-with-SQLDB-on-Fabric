...
1. Enter your AAD Username e.g. `AIAP.<YourCustomUserName>@fabconatl26.onmicrosoft.com` in the Email field, then click on the Submit button.
   > **During the workshop, you should have received a printed one-pager from the proctor or speaker. Replace `<YourCustomUserName>` with the value provided on that sheet. Then, enter or copy the AAD user from the page, for example: AIAP.user001@fabconatl26.onmicrosoft.com**
...
> **Note:** The password provided for this lab is a **temporary password**. Please change it immediately to a **password** you can easily remember or securely note down for future use.


!["code"](../../media/presign.png)

5. If popup "Welcome to the Fabric view" is showed, feel free to close it by selecting 'X' on the top right corner and proceed with the workshop content.
...
2. In the **Create a workspace blade**, enter a unique name for the **Workspace Name** field.
    >**Note:** To ensure your workspace name is unique, add the suffix from your username to the workspace name. For example, if your username is ``AIAP.user001@fabcon26.onmicrosoft.com``, use ``FabricWorkspaceuser001`` as the workspace name (where ``user001`` is taken from your username).
...
2. In the **New item** blade on the right,  use the **Filter by item type search box** in the upper right to enter **SQL**
...
3. With the New item results filtered down, click on the **SQL database ** tile.
...
> [!IMPORTANT]
> There may be a few seconds delay after pressing the **SQL database tile** and when the **New SQL database modal** appears. Just give it a few seconds if it does not appear immediately. 
...
## Section 3: Set up Auditing for SQL database in Fabric

Auditing SQL databases in Fabric is a critical security and compliance feature that enables organizations to track and log database activities. Auditing supports compliance, threat detection, and forensic investigations by helping to answer questions like who accessed what data, when, and how. 

Enable Auditing for your Fabric database using Fabric portal. Currently auditing for Fabric SQL database is not enabled by default.  

By default, the Audit everything option captures all events including batch completions, successful and failed authentication. 
...
To enable auditing go to Security tab and click Manage SQL Auditing.

 <img alt="A picture of drop-down list of SQL database and SQL analytics endpoint available in the top right corner of the Database explorer in the Fabric portal" src="../../media/Auditing/AuditingSelection.png" style="width:400px;">

Turn on "Automatically save database events to SQL audit logs stored in OneLake"
For Record these event types- select "Data reads and writes"

<img alt="A picture of drop-down list of SQL database and SQL analytics endpoint available in the top right corner of the Database explorer in the Fabric portal" src="../../media/Auditing/AuditingSetup.png" style="width:400px;">
Auditing is now set up and will effectively capture both read and write operations. You will review the logs in later section.

## Section 4: Working with the SQL database in Microsoft Fabric

In this next section, you will be focused on using the Database Explorer and SQL query blades in Microsoft Fabric

### Task 4.1: Getting familiar with Database Explorer and Query editor
...
### Task 4.2: Querying the Audit logs for SQL database in Fabric

After performing data operations within the database, you can retrieve audit logs by utilizing the T-SQL function sys.fn_get_audit_file_v2.  

Audit logs are stored in the audit folder located within the One Lake artifact directory.   
> **Note:** Replace <fabric_workspace_id> with your Fabric workspace ID. You can find the ID of a workspace easily in the URL, it's the unique string inside two / characters after /groups/ in your browser window. 
  <img alt="A picture of fabric id" src="../../graphics/Introduction/GetworkspaceID.png" style="width:400px;">

> **Note:** Replace <fabric SQL database id> with your SQL database in Fabric database ID. You can find the ID of the database item easily in the URL, it is the unique string inside two / characters after /SQL database/ in your browser window. 
  <img alt="A picture of SQL id" src="../../graphics/Introduction/GetSQLID.png" style="width:400px;">
...
  'https://onelake.blob.fabric.microsoft.com/<fabric_workspace_id>/<fabric_sql_database_id>/Audit/sqldbauditlogs/', 
...
### Task 4.3: Mirroring Fabric SQL database to OneLake
...
1. To access SQL analytics endpoint, you can switch to SQL analytics endpoint mode in SQL database in Database explorer in the top right corner.
   >**Note** If SQL analytics endpoint is not enabled then click ctrl + F5 to refresh the page
...
3. Another way to access SQL analytics endpoint is from the workspace view. Let's open the workspace by selecting a workspace in the menu on the left side.
...
4. You can now observe that there's a SQL analytics endpoint available below your SQL database in Fabric item, like on the image below.
...
5. Let's select a SQL analytics endpoint item to open it, then select a **New SQL query** and execute the following query:
...
6. Let's now get back to SQL database by choosing a "SQL database" from the top right dropdown.
...
7. In the SQL database query editor select _"New query"_ and execute the following query to insert one record:
    ...
    ```
8. You can monitor the replication status of your database by navigating to the _Replication_ tab on top left corner and selecting _Monitor replication_.
...
9. This will open up a blade on the right side that demonstrates the status and the last mirroring refresh time.
...
10. Switch back to SQL Analytics endpoint through a dropdown on top right corner
...
11. And now rerun the same query we used before in SQL Analytics endpoint and observe that the first record on top is the one inserted via SQL database.
...
[Use Copilot with SQL database in Fabric](../Module%2002%20-%20Explore%20AI%20and%20Copilot%20capabilities/Copilot%20capabilities%20for%20SQL%20database%20in%20Microsoft%20Fabric.md) to enhance your T-SQL queries and boost your productivity.
