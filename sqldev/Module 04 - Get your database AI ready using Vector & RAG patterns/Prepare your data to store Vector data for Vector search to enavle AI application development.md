...
## Section 1: Setting up SQL Database in Fabric for Semantic Retrieval using Vector

### Task 1.1: Setup of database credential
...
> **Note:** During this lab, the OpenAI API key and API name will be provided. If you need assistance accessing these details, please reach out to the proctor. Replace ``<your-api-endpoint>`` with the name of your **Azure OpenAI** service and ``<api-key>`` with the **API key** for the Azure OpenAI API.
...
> [!IMPORTANT]
>
> **Replace ``<your-api-endpoint>`` and ``<api-key>``with your Open AI enpoint and Key**
...
'<your-api-endpoint>')
BEGIN
	DROP database scoped credential [<your-api-endpoint>];
END
CREATE database scoped credential [<your-api-endpoint>]
with identity = 'HTTPEndpointHeaders', secret = '{"api-key": "<api-key>"}';
go
```

## Section 2: Creating embeddings for relational data
...
### Task 2.2: Preparing the database and creating embeddings
...
### Task 2.2: Create a stored procedure to create embeddings

Below stored procedure would take an input text and output of type vector
...
```SQL
   ...
   AS
   BEGIN
   declare @url varchar(max) = '<your-api-endpoint>/openai/deployments/text-embedding-ada-002/embeddings?api-version=2024-06-01';
...
2. Run the following T-SQL in a new query window to create embeddings for all products in the Products table:
...
3. To ensure all the embeddings were created, run the following code in a new query window: 
...
4. Run the next query in a new query window to see the results of the above update to the Products table:
...
!["A picture of the query result showing the chunk and embeddings columns and their data." ](../../media/2025-01-15_6.34.32_AM.png)

## Section 3: Vector similarity searching
...
**Query 1**
The first query will pose the question "I am looking for a red bike and I dont want to spend a lot". The key words that should help with our similarity search are red, bike, and dont want to spend a lot. Run the following SQL in a new query window:
...
!["A picture of running Query 1 and getting results outlined in the Query 1 results table." ](../../media/2025-01-15_6.36.01_AM.png)

**Query 2**
In the previous example, we were clear on what we were looking for; cheap red bike. In this next example, you are going to have the search flex its AI muscles a bit by saying we want a bike seat that needs to be good on trails. This will require the search to look for adjacent values that have something in common with trails. 
...
## Section 4: Stored Procedure to find products
### Task 4.1: Create a new stored procedure to find products. 

**Stored Procedure to find products with a prompt**
...
**Stored Procedure to use in API calls**
...
## Task 4.2: Test 
Let us test this newly created procedure to see the results by running the following SQL in a new query window:

```SQL
EXEC SalesLT.find_products_api 'I am looking for a red bike'
```
!["A picture of running the find_products_api stored procedure"](../../media/2025-01-14_6.57.09_AM.png)



## What's next
Congratulations! You have learnt how to get your data ready for  **Vector Seach** to enhance your **database AI development** experience with open AI text embedding model. With these skills, you are now better equipped to create embeddings and use vector search. This is the building blocks for RAG pattern applications . You are ready to move on to the next exercise: 
[Create GRaphQL API for Chatbot Application](../Module%2005%20-%20Create%20GraphQL%20API%20for%20Chatbot%20Application/Create%20GraphQL%20API%20for%20RAG%20Application.md).
